...
int main(int argc, char** argv)
{
    ...
    addr_t list_head = 0, cur_table_entry = 0;
    addr_t sys_call_ptr = 0;
    unsigned long sys_call_ptr_offset = 0;
    int table_poll_size = 0, i = 0;
    ...
    table_poll_size = 300;
    sys_call_ptr_offset = sizeof(void*);
    ...
    if (VMI_FILE != mode)
    {
        ...
        printf("Syscall Table listing for VM %s (id=%" PRIu64 ")\n", name2, id);
    }
    else
    {
        printf("Syscall Table listing for file %s\n", name2);
    }
    free(name2);
    ...
    if (VMI_OS_LINUX == os)
    {
         ...
         */
        if (VMI_FAILURE ==
            vmi_translate_ksym2v(vmi, "sys_call_table", &list_head))
            goto error_exit;
    }
    else
    {
    ...
    cur_table_entry = list_head;

    /* walk the syscall table */
    for (i = 0; i < table_poll_size; i++)
    {
        vmi_read_addr_va(vmi, cur_table_entry, 0, &sys_call_ptr);
        printf("[%d] syscall func addr: %" PRIx64 "\n", i, sys_call_ptr);
        cur_table_entry += sys_call_ptr_offset;
    }
