...
int main(int argc, char** argv)
{
    ...
    /* memory area related variables. note that proc_mm may be null. */
    addr_t proc_mm = 0;
    unsigned long mm_offset = 0;
    /* in include/linux/mm_types.h -- mm_struct */
    addr_t mm_struct_mmap = 0; /* list of VMAs */
    /* in vm_area_struct */
    unsigned long vm_start = 0, vm_end = 0;
    unsigned long vm_page_prot = 0;
    addr_t vm_next = 0;
    unsigned long vm_start_offset = 0, vm_end_offset = 0;
    unsigned long vm_page_prot_offset = 0;
    unsigned long vm_next_offset = 0;
    unsigned long vm_area_count = 0;
    ...
    /* init mm related variables */
    mm_offset = 2040;
    vm_start_offset = 0;
    vm_end_offset = 8;
    vm_page_prot_offset = 72;
    vm_next_offset = 16;
    ...
    if (VMI_FILE != mode)
    {
        ...
        printf("Check per-process memory area for VM %s (id=%" PRIu64 ")\n",
               name2, id);
    }
    else
    {
        printf("Check per-process memory area for file %s\n", name2);
    }
    free(name2);
    ...
    while (1)
    {
        ...
        /* read mm info in current process.
         * Note that mm may be NULL. So we should check before using it.
         */
        vmi_read_addr_va(vmi, current_process + mm_offset, 0, &proc_mm);

        if (proc_mm > 0)
        {
            /* read mmap info in current mm_struct */
            vmi_read_addr_va(vmi, proc_mm, 0, &mm_struct_mmap);

            /* walk the vm_area_struct list */
            while (1)
            {
                /* read vm_area info in current vm_area_struct mmap */
                vmi_read_64_va(vmi, mm_struct_mmap + vm_start_offset, 0,
                               (uint64_t*)&vm_start);
                vmi_read_64_va(vmi, mm_struct_mmap + vm_end_offset, 0,
                               (uint64_t*)&vm_end);
                vmi_read_64_va(vmi, mm_struct_mmap + vm_page_prot_offset, 0,
                               (uint64_t*)&vm_page_prot);

                /* print out vm_area info */
                vm_area_count++;
                printf("[%5d -- %ld] start addr : %" PRIx64
                       ", end addr: %" PRIx64 ", page permission: %lu\n",
                       pid, vm_area_count, vm_start, vm_end, vm_page_prot);

                /* check if the vm_next is NULL */
                vmi_read_addr_va(vmi, mm_struct_mmap + vm_next_offset, 0,
                                 &vm_next);
                if (vm_next == 0)
                {
                    break;
